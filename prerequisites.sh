...
curl -LO https://dl.k8s.io/release/v1.29.0/bin/linux/amd64/kubectl || handle_error "Failed to download kubectl"
curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl.sha256" || handle_error "Failed to download kubectl sha256"
echo "$(cat kubectl.sha256)  kubectl" | sha256sum --check || handle_error "Failed to verify kubectl"
sudo install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl || handle_error "Failed to install kubectl"
sudo rm -rf kubectl && kubectl version --client || handle_error "Failed to verify kubectl version"
